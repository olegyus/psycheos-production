...
Кейсы различаются не сюжетом, а ДИНАМИКОЙ:
  Case 1: высокий L2, средний L0, доступный L3, умеренная толерантность к интерпретациям
  Case 2: нестабильный L0, слабый L3, частые скачки, низкая толерантность к неопределённости
  Case 3: высокий baseline L0, узкое когнитивное окно, быстрый S4, узкий диапазон интервенций
"""
...
# ═══════════════════════════════════════════════════════════════════════════
# Динамика: высокий L2, средний L0, доступный L3,
#           умеренная толерантность к интерпретациям
# Специалисту «кажется», что клиент готов к глубинной работе —
# но L2 защищает систему рационализацией. Ловушка интеллектуализации.
...
CASE_1 = BuiltinCase(
    ...
    layers=Layers(
        ...
        L3=LayerDescription(
            description="Идентичность «компетентного и надёжного». «Если провалюсь — я не тот, за кого себя выдаю». Скрытый синдром самозванца. Доступен при аккуратной работе — клиент способен к рефлексии, если L0 стабилен.",
    ...
    dynamics=CaseDynamics(
        ...
        L0_reactivity="moderate",       # L0 средний — растёт, но не взрывается
        L2_strength="high",             # КЛЮЧЕВОЕ: мощная рационализация
        L3_accessibility="high",        # КЛЮЧЕВОЕ: L3 доступен — ловушка
        interpretation_tolerance="moderate",  # КЛЮЧЕВОЕ: принимает, но L2 перехватывает
        uncertainty_tolerance="moderate",
        cognitive_window="wide",        # Может думать — проблема не в доступе, а в защите
        escalation_speed="slow",        # Медленный переход в S4
        intervention_range="wide",      # Много допустимого — но мало эффективного

        recovery_rate=0.7,              # Быстро восстанавливается
        volatility=0.2,                 # Стабильный, предсказуемый
    ),
...
# ═══════════════════════════════════════════════════════════════════════════
# Динамика: нестабильный L0, слабый L3, частые скачки,
#           низкая толерантность к неопределённости
# Система «прыгает» — специалисту трудно удержать фокус.
# Любая пауза = триггер. Любая интерпретация = эскалация.
...
CASE_2 = BuiltinCase(
    ...
    layers=Layers(
        ...
        L3=LayerDescription(
            description="СЛАБО дифференцированная идентичность. Зависит от подтверждения через другого. «Отдаляется — я не ценна». L3 легко захлёстывается аффектом — рефлексия доступна только при низком L0.",
    ...
    dynamics=CaseDynamics(
        ...
        L0_reactivity="high",            # КЛЮЧЕВОЕ: L0 нестабильный, быстро растёт
        L2_strength="low",               # КЛЮЧЕВОЕ: слабые стратегии, легко ломаются
        L3_accessibility="low",           # КЛЮЧЕВОЕ: L3 захлёстывается аффектом
        interpretation_tolerance="low",   # КЛЮЧЕВОЕ: интерпретации = эскалация
        uncertainty_tolerance="low",      # КЛЮЧЕВОЕ: не выносит неопределённости
        cognitive_window="moderate",      # Окно есть, но нестабильно
        escalation_speed="fast",          # Быстрый переход в S4
        intervention_range="moderate",    # Есть варианты, но нужна точность

        recovery_rate=0.4,               # Медленное восстановление
        volatility=0.8,                  # КЛЮЧЕВОЕ: частые скачки параметров
    ),
...
# ═══════════════════════════════════════════════════════════════════════════
# Динамика: высокий baseline L0, узкое когнитивное окно,
#           быстрый S4, узкий диапазон допустимых интервенций
# «Коридор» — почти любое действие, кроме стабилизации L0, ведёт к красному.
# Тест на способность НЕ делать лишнего.
...
CASE_3 = BuiltinCase(
    ...
    dynamics=CaseDynamics(
        ...
        L0_reactivity="high",            # Высокий baseline + быстро растёт
        L2_strength="low",               # Стратегии разрушены
        L3_accessibility="low",           # НЕДОСТУПЕН — обращение усиливает кризис
        interpretation_tolerance="low",   # Любая интерпретация = красный
        uncertainty_tolerance="low",      # Не выносит
        cognitive_window="narrow",        # КЛЮЧЕВОЕ: узкое окно, мало что проходит
        escalation_speed="fast",          # КЛЮЧЕВОЕ: быстрый S4
        intervention_range="narrow",      # КЛЮЧЕВОЕ: только стабилизация L0 работает

        recovery_rate=0.25,              # Очень медленное восстановление
        volatility=0.5,                  # Умеренная — не хаотичный, а застывший
    ),
