...
# ═══════════════════════════════════════════════════════════════════════════
# ALTERNATIVE TRAJECTORY (v1.1) — блок альтернативных сценариев
# ═══════════════════════════════════════════════════════════════════════════
...
class SpecialistProfile(BaseModel):
    ...
    # Список TSI по сессиям (для отслеживания динамики)
...
class HiddenState(BaseModel):
    ...
    trust_level: int = 30
    defense_activation: int = 40         # v1.1: новая переменная
    active_layer: str = "L0"             # v1.1: текущий активный слой
...
class SessionData(BaseModel):
    ...
    # История сообщений для Claude
    messages: list[dict] = Field(default_factory=list)

    # v1.0 логи (сохраняем для обратной совместимости)
    signal_log: list[str] = Field(default_factory=list)
    fsm_log: list[str] = Field(default_factory=list)

    # v1.1: структурированный лог итераций
    iteration_log: list[IterationLog] = Field(default_factory=list)

    # v1.1: TSI (заполняется при /end)
    tsi: Optional[TSIComponents] = None

    # v1.1: альтернативные траектории (заполняются при /end)
