...
class ParsedResponse:
    ...
    active_layer: Optional[str] = None     # L0–L4
    match_score: float = 0.0               # regulatory_match 0–1
    cascade_prob: float = 0.0              # cascade probability 0–1
    delta_trust: int = 0
    delta_tension: int = 0
...
def parse_claude_response(raw: str) -> ParsedResponse:
    ...
    # Разделяем на клиента и супервизора
    parts = re.split(r'\n-{3,}\n', raw, maxsplit=1)
    ...
    # ── REASON (маркер — первая строка после DELTA/CRISIS) ─────────────
    lines = block.split("\n")
    reason_lines = []
    started = False
    for line in lines:
...
def build_iteration_log(
    ...
    return IterationLog(
        ...
        specialist_input=specialist_input[:200],  # ограничиваем длину
...
def format_for_telegram(parsed: ParsedResponse) -> str:
    ...
    # Кризисное предупреждение (дополнительное выделение)
