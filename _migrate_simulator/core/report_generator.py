"""Генератор отчёта v1.1 — текст Claude + метрики → .docx файл.
...
from data.schemas import (
    ...
    SpecialistProfile, AlternativeTrajectory,
)
...
def generate_report_docx(
    ...
    """Генерирует .docx отчёт. Возвращает путь к файлу."""
    ...
    # ── Таблица сигналов (из iteration_log) ────────────────────────────
    if iteration_log:
        _add_signal_table(doc, iteration_log)
        doc.add_page_break()

    # ── Основной текст отчёта от Claude ────────────────────────────────
    ...
    filename = f"PsycheOS_Report_{case_id}_{timestamp}.docx"
...
def _add_tsi_block(doc, tsi: TSIComponents):
    """Добавляет блок TSI с 5 компонентами."""
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
...
def _add_signal_table(doc, iteration_log: list[IterationLog]):
    """Добавляет таблицу сигналов по репликам."""
...
def _add_specialist_profile(doc, profile: SpecialistProfile):
    """Добавляет блок накопительного профиля специалиста."""
